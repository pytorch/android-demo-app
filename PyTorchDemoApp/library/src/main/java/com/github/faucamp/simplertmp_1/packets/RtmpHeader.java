...
import com.github.faucamp.simplertmp_1.io.RtmpSessionInfo;
    ...
    public enum MessageType {
        ...
        public static MessageType valueOf(byte messageTypeId) {
            if (quickLookupMap.containsKey(messageTypeId)) {
                ...
                throw new IllegalArgumentException("Unknown message type byte: " + FaucampUtil.toHexString(messageTypeId));
            }
        }
    }
    ...
    public enum ChunkType {
        ...
        public static ChunkType valueOf(byte chunkHeaderType) {
            if (quickLookupMap.containsKey(chunkHeaderType)) {
                ...
                throw new IllegalArgumentException("Unknown chunk header type byte: " + FaucampUtil.toHexString(chunkHeaderType));
            }
        }
    }
    ...
    private void readHeaderImpl(InputStream in, RtmpSessionInfo rtmpSessionInfo) throws IOException {
        ...
        switch (chunkType) {
            case TYPE_0_FULL: { //  b00 = 12 byte header (full header) 
                ...
                absoluteTimestamp = FaucampUtil.readUnsignedInt24(in);
                timestampDelta = 0;
                // Read bytes 4-6: Packet length
                packetLength = FaucampUtil.readUnsignedInt24(in);
                ...
                FaucampUtil.readBytesUntilFull(in, messageStreamIdBytes);
                messageStreamId = FaucampUtil.toUnsignedInt32LittleEndian(messageStreamIdBytes);
                // Read bytes 1-4: Extended timestamp
                extendedTimestamp = absoluteTimestamp >= 0xffffff ? FaucampUtil.readUnsignedInt32(in) : 0;
            ...
            case TYPE_1_RELATIVE_LARGE: { // b01 = 8 bytes - like type 0. not including message stream ID (4 last bytes)
                ...
                timestampDelta = FaucampUtil.readUnsignedInt24(in);
                // Read bytes 4-6: Packet length
                packetLength = FaucampUtil.readUnsignedInt24(in);
                // Read byte 7: Message type ID
                messageType = MessageType.valueOf((byte) in.read());
                // Read bytes 1-4: Extended timestamp delta
                extendedTimestamp = timestampDelta >= 0xffffff ? FaucampUtil.readUnsignedInt32(in) : 0;
            ...
            case TYPE_3_RELATIVE_SINGLE_BYTE: { // b11 = 1 byte: basic header only 
                RtmpHeader prevHeader = rtmpSessionInfo.getChunkStreamInfo(chunkStreamId).prevHeaderRx();
                // Read bytes 1-4: Extended timestamp
                extendedTimestamp = prevHeader.timestampDelta >= 0xffffff ? FaucampUtil.readUnsignedInt32(in) : 0;
            ...
            default:
                throw new IOException("Invalid chunk type; basic header byte was: " + FaucampUtil.toHexString((byte) basicHeaderByte));
        }
    }
    ...
    public void writeTo(OutputStream out, ChunkType chunkType, final ChunkStreamInfo chunkStreamInfo) throws IOException {
        ...
        switch (chunkType) {
            case TYPE_0_FULL: { //  b00 = 12 byte header (full header)
                ...
                FaucampUtil.writeUnsignedInt24(out, (absoluteTimestamp >= 0xffffff) ? 0xffffff : absoluteTimestamp);
                FaucampUtil.writeUnsignedInt24(out, packetLength);
                out.write(messageType.getValue());
                FaucampUtil.writeUnsignedInt32LittleEndian(out, messageStreamId);
                if (absoluteTimestamp >= 0xffffff) {
                    extendedTimestamp = absoluteTimestamp;
                    FaucampUtil.writeUnsignedInt32(out, extendedTimestamp);
                }
                break;
            }
            case TYPE_1_RELATIVE_LARGE: { // b01 = 8 bytes - like type 0. not including message ID (4 last bytes)
                ...
                FaucampUtil.writeUnsignedInt24(out, (absoluteTimestamp >= 0xffffff) ? 0xffffff : timestampDelta);
                FaucampUtil.writeUnsignedInt24(out, packetLength);
                out.write(messageType.getValue());
                if (absoluteTimestamp >= 0xffffff) {
                    extendedTimestamp = absoluteTimestamp;
                    FaucampUtil.writeUnsignedInt32(out, absoluteTimestamp);
                }
                break;
            }
            case TYPE_2_RELATIVE_TIMESTAMP_ONLY: { // b10 = 4 bytes - Basic Header and timestamp (3 bytes) are included
