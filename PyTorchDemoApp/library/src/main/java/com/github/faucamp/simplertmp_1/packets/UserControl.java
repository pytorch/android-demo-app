...
public class UserControl extends RtmpPacket {
    ...
    public void readBody(InputStream in) throws IOException {
        ...
        type = Type.valueOf(FaucampUtil.readUnsignedInt16(in));
        int bytesRead = 2;
        // Event data (1 for most types, 2 for SET_BUFFER_LENGTH)
        if (type == Type.SET_BUFFER_LENGTH) {
            setEventData(FaucampUtil.readUnsignedInt32(in), FaucampUtil.readUnsignedInt32(in));
            bytesRead += 8;
        } else {
            setEventData(FaucampUtil.readUnsignedInt32(in));
            bytesRead += 4;
        }
    ...
    protected void writeBody(OutputStream out) throws IOException {
        ...
        FaucampUtil.writeUnsignedInt16(out, type.getIntValue());
        // Now write the event data
        FaucampUtil.writeUnsignedInt32(out, eventData[0]);
        if (type == Type.SET_BUFFER_LENGTH) {
            FaucampUtil.writeUnsignedInt32(out, eventData[1]);
        }
    }
